...
- Hermes terminal home isolation sets `HOME=/opt/data/home` while `HERMES_HOME=/opt/data`; tool dirs like `.agent-browser`, `.wrangler`, `.config/turso`, `.config/bws`, and `.bws` symlink to canonical paths under `/opt/data`.
...
- Baked-in CLIs include gh, deno, wrangler, turso, agent-browser, Cursor Agent (`agent`), Bitwarden Secrets Manager (`bws`), PostgreSQL client (`pg_dumpall`, `pg_dump`, `psql`), zip/unzip, nano, and Python packages faster-whisper and exa-py.
