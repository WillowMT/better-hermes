...
- `better-hermes` extends `nousresearch/hermes-agent:v2026.6.19` via a custom Dockerfile; compose builds `better-hermes:v2026.6.19` from the repo root.
...
- `docker-compose.yml` passes tokens/secrets from the host `.env`; `.env.example` documents expected variables.
- Baked-in CLIs include gh, gcloud, deno, vercel, wrangler, turso, agent-browser, zip/unzip, nano, and Python packages faster-whisper and exa-py.
