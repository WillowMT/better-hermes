...
- Hermes terminal home isolation sets `HOME=/opt/data/home` while `HERMES_HOME=/opt/data`; tool dirs like `.agent-browser`, `.config/gcloud`, `.wrangler`, `.config/turso`, `.config/bws`, and `.bws` symlink to canonical paths under `/opt/data`.
- `docker-compose.yml` passes tokens/secrets from the host `.env` (including `CURSOR_API_KEY` and `BWS_ACCESS_TOKEN`); `.env.example` documents expected variables.
- Baked-in CLIs include gh, gcloud, deno, vercel, wrangler, turso, agent-browser, Cursor Agent (`agent`), Bitwarden Secrets Manager (`bws`), PostgreSQL client (`pg_dumpall`, `pg_dump`, `psql`), zip/unzip, nano, and Python packages faster-whisper and exa-py.
